...
class PER:
    ...
    def add(self, experience: Exp, error: float):
        idx = self.size + self.capacity - 1
        self.data[self.size] = experience
        self.size = min(self.size + 1, self.capacity)
        self.update(idx, error)

    def update(self, idx: int, error: float):
        priority = (error + self.ε) ** self.α
    ...
    def sample(self, batch_size: int) -> Tuple[List[Exp], List[int], np.ndarray]:
        ...
        weights = (self.size * sampling_probabilities) ** -self.β
        weights /= weights.max()
...
class DuelingDQN(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, hidden_dim: List[int]):
        ...
        self.feature = nn.Sequential(
            ...
            nn.ReLU(),
            NoisyLinear(hidden_dim[0], hidden_dim[1]),
            nn.ReLU()
        )
        self.advantage = nn.Sequential(
            NoisyLinear(hidden_dim[1], hidden_dim[2]),
            nn.ReLU(),
...
class Trainer:
    ...
    def n_step_learn(self):
        if len(self.n_step_buffer) < self.agent.n_step:
            return
    ...
    def evaluate(self, num_episodes: int = 100) -> float:
        return np.mean([
            sum(self.env.step(self.agent.act(s))[1] for s in [self.env.reset()] for _ in iter(lambda: self.env.step(self.agent.act(s))[2], True))
